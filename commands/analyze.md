---
description: Analyze a stock with fundamentals and price action
arguments:
  - name: ticker
    description: Stock ticker symbol (e.g., AAPL, MSFT, NVDA)
    required: true
---

# Stock Analysis: $1

Perform a comprehensive analysis of **$1** using the investor-agent tools.

## Analysis Steps

1. **Get comprehensive ticker data** including:
   - Key metrics (P/E, market cap, margins, growth rates)
   - Upcoming events (earnings, dividends)
   - Analyst recommendations

2. **Get price history** (1 month) to understand recent price action

3. **Get institutional holders** to see who the major investors are

4. **Synthesize findings** into actionable insights:
   - Valuation assessment (cheap/fair/expensive relative to peers)
   - Key risks and catalysts
   - Summary recommendation

Present the analysis in a clear, structured format suitable for investment research.
